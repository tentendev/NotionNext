...
import Progress from '@/theme/custom/components/Progress'
import TagItem from '@/theme/custom/components/TagItem'
import formatDate from '@/lib/formatDate'
import { Code, Collection, CollectionRow, Equation, NotionRenderer } from 'react-notion-x'
import ShareBar from '@/theme/custom/components/ShareBar'
import Comment from '@/theme/custom/components/Comment'
...
import RecommendPosts from '@/theme/custom/components/RecommendPosts'
import TocDrawer from '@/theme/custom/components/TocDrawer'
import TocDrawerButton from '@/theme/custom/components/TocDrawerButton'
...
export default function ArticleLayout ({ post, blockMap, tags, prev, next, allPosts, categories }) {
  if (!post) {
    return <Custom404 />
  }
  const meta = {
    ...
    <div id='article-wrapper' ref={targetRef} className='flex-grow'>
      ...
      <div className='max-w-5xl mx-auto mt-16 xl:mt-32 w-screen md:w-full duration-300'>
        ...
        <article itemScope itemType='https://schema.org/Movie'
                 className='hover:shadow-2xl pt-10 animate__fadeIn animate__animated subpixel-antialiased lg:pt-32 lg:px-44 px-5 py-2 dark:border-gray-700 bg-white dark:bg-gray-800'>
          {post.type && !post.type.includes('Page') && (<>
            <header className='w-full h-60 lg:h-96 transform duration-200 md:flex-shrink-0 overflow-hidden'>
              <Image src={(post.page_cover && post.page_cover.length > 1) ? post.page_cover : BLOG.defaultImgCover}
                     loading='eager' objectFit='cover' layout='fill' alt={post.title} />
            </header>
          </>)}
          {/* 文章Title */}
          ...
          <section className='flex-nowrap flex  mt-1 dark:text-white font-light'>
            <Link href={`/category/${post.category}`} passHref>
              <div
                className='cursor-pointer text-md py-2 ml-1 mr-3 text-gray-500 dark:text-gray-300 hover:text-black dark:hover:text-white'>
            ...
            {post.type[0] !== 'Page' && (
              <Link href={`/archive#${post?.date?.start_date?.substr(0, 7)}`} passHref>
                <div
                  className='pl-1 cursor-pointer hover:text-gray-700  dark:hover:text-gray-200 text-gray-400 dark:text-gray-400 leading-10'>
                  {date}
                </div>
              </Link>
            )}
